...
- [ ] T008 [P] Create `src/web/state/gameStore.ts` — `subscribe(cb)` / `getSnapshot()` adapter over the mutable `Game` instance with a structurally-memoised stable `GameSnapshot` (research.md R1, `contracts/app-state.md` §GameSnapshot)
- [ ] T009 Create `src/web/state/selectors.ts` — memoised selector hooks `useProgress`, `useStations`, `useStation(id)`, `usePuzzle(id)`, `useRecommendedNext`, `usePets`, each passing a slice selector to `useSyncExternalStore` (data-model.md §3e)
- [ ] T010 Create `src/web/state/sessionReducer.ts` — `SessionState` + `SessionAction` union (`SUBMIT_RESULT`, `ENTER_PRACTICE`, `EXIT_PRACTICE`, `OPEN_INFO`, `CLOSE_MODAL`, `PUSH_TOAST`, `EXPIRE_TOAST`, `DISMISS_INTRO`, `RESET`), pure (data-model.md §3b)
- [ ] T011 [P] Create `tests/component/sessionReducer.test.ts` — pure reducer coverage for every action incl. `RESET` clearing session state
- [ ] T012 Create `src/web/state/actions.ts` (or co-locate in provider) — `GameActions`: `submitPuzzle`, `practiceSubmit`, `recordQuizPass`, `reset`, `markIntroSeen`, `autoSolveJourney`; each delegates to the domain (`Game.completePuzzle` / `Game.practiceComplete` / `validatePuzzleInput`), returns `SubmitResult`, is idempotent for already-solved puzzles, batches one store notification on success (`contracts/app-state.md` §GameActions)
- [ ] T013 Create `src/web/state/GameProvider.tsx` — mounts the store, `useReducer(sessionReducer)`, and the three split contexts `GameStoreContext` / `GameActionsContext` / `SessionContext` (research.md R3, `contracts/app-state.md` §Contexts)
- [ ] T014 [P] Create `src/web/state/useReducedMotion.ts` — `matchMedia('(prefers-reduced-motion: reduce)')` hook (FR-047)

### Navigation

- [ ] T015 [P] Create `src/web/app/routes.ts` — `Route` union (`intro | studio | station | puzzle | collection | grand-canvas`), pure total `parseHash(hash)` and `serialiseRoute(route)`, plus the guard-resolution table (locked/unknown → `studio`; `grand-canvas` without unlock → `studio`) from `contracts/app-state.md` §Guard rules
- [ ] T016 [P] Create `tests/component/useHashRoute.test.ts` — `parseHash` round-trips, unparseable → `studio`, every guard-rule row
- [ ] T017 Create `src/web/app/useHashRoute.ts` — hook returning guard-resolved `route` + `navigate(route)` (sets `location.hash`), driven by `hashchange` and the current `GameSnapshot` (research.md R2)

### Persistence

- [ ] T018 Extend `src/web/localProgress.ts` — add optional `introSeen?: boolean` and `lastRoute?: string` to `LocalProgressSnapshot`, with tolerant per-field parsing (wrong type → dropped, never throws) per `contracts/persistence.md` §Read contract; keep key `ctg:web-progress:v1`
- [ ] T019 [P] Update `tests/localProgress.test.ts` — new-field parsing, unknown extra keys ignored, corrupt JSON → `null`, `setItem` throw swallowed
- [ ] T020 Create `src/web/state/persistenceSync.ts` — on mount `readLocalProgress()` → rebuild `Game` by replaying `completedPuzzleIds` through the domain restore path → restore `learningProgressByPuzzle` / `activeStationId` / `practicePuzzleId` / `introSeen`; on first mount resolve and set the initial `location.hash` per `contracts/persistence.md` §4 (`lastRoute` if it passes `parseHash` + guards, else `activeStationId` → that station, else `studio`; `intro` only when `introSeen !== true` and no progress); debounced (≥250 ms) `saveLocalProgress()` on snapshot/session change; never writes before first progress on a fresh run (`skipNextPersist`); swallows storage errors (`contracts/persistence.md` §Load/restore sequence, FR-049–FR-051)
- [ ] T021 [P] Create `tests/component/persistenceSync.test.ts` — `tests/fixtures/legacy-save-v1.json` restores with zero loss; missing `introSeen` → first run; corrupt JSON → fresh game, blob not overwritten until progress; `localStorage` throw → session continues; reset then reload → fresh state; initial-route resolution (valid `lastRoute` honoured; invalid `lastRoute` falls back to `activeStationId`; no progress + no `introSeen` → `intro`) (`contracts/persistence.md` §Compatibility tests)
...
- [ ] T038 [P] [US1] Build `src/web/components/HUD.tsx` — station/completion/pets/streak/Grand-Canvas summary via `useProgress`, plus pet-milestone badges (Color Apprentice / Palette Keeper / Chromatic Master) from `progress.petMilestonesUnlocked` rendered with icon + label (not colour alone); simplifies (not shrinks) on small screens (FR-015, FR-030, FR-035, SC-002)
- [ ] T039 [P] [US1] Build `src/web/components/AppMenu.tsx` — design-system `Menu`: "Reset run", "Replay intro", "Feedback"; "Auto solve journey" shown only in dev/e2e (FR-021, FR-022, FR-030a)
- [ ] T040 [P] [US1] Build `src/web/screens/IntroScreen.tsx` — 3–4 caretaker lines + "Enter the Studio" + "Skip"; calls `actions.markIntroSeen()`; only shown when `introSeen !== true` and no progress (FR-030a, `contracts/persistence.md`)
- [ ] T041 [US1] Fill `src/web/screens/StudioScreen.tsx` (functional, not yet redesigned) — station list from `useStations`, enter/continue navigation, basic recommended-next line
- [ ] T042 [US1] Fill `src/web/screens/StationScreen.tsx` — puzzle list/map from `useStation(id)`, "Back to Studio", "Go to <next station>" CTA when complete (FR-024, FR-027)
- [ ] T043 [P] [US1] Build `src/web/components/LearningIntro.tsx` and `src/web/components/LearningQuiz.tsx` — port copy/behaviour from `src/web/legacy/learningFlow.ts`; 100%-pass gate; `onPass()` → `actions.recordQuizPass` (FR-016, US3-2)
- [ ] T044 [US1] Fill `src/web/screens/PuzzleScreen.tsx` — local `useReducer` learning stage (`intro | quiz | solve`, initial `solve` if quiz already passed); renders `LearningIntro` → `LearningQuiz` → `PuzzlePlayer`; "How this works" opens `InfoModal` (FR-031)
- [ ] T045 [US1] Build `src/web/components/PuzzlePlayer.tsx` — owns `useState(value)`, renders `LegacyPuzzleAdapter` for now + a real design-system `<CheckButton>` in-subtree; `submit()` → `actions.submitPuzzle`; practice mode → `actions.practiceSubmit` with cap/streak messaging; double-submit guard (`contracts/puzzle-component.md` §PuzzlePlayer, FR-006, FR-019)
- [ ] T046 [P] [US1] Build `src/web/components/ResultPanel.tsx` — consumes `src/web/puzzles/diagnose.ts` + `failureReasons.ts`; `alert`/`status` region starting "Correct"/"Not quite"; "Try again"; calls `announce()` (FR-017, FR-034, FR-036)
- [ ] T047 [P] [US1] Build `src/web/components/RewardReveal.tsx` — pet reveal + score/HUD update + encouraging message; brief, non-blocking (FR-033; polish in US4)
- [ ] T047a [P] [US1] Build `src/web/components/ToastHost.tsx` — renders the `sessionReducer` `toasts` queue (station-complete, practice points, pet-unlock echo) as transient auto-expiring messages (`EXPIRE_TOAST`), mounted once in `App.tsx`; reward/pet celebration stays in `RewardReveal` (spec Baseline "Toasts", FR-036)
- [ ] T048 [US1] Build `src/web/components/InfoModal.tsx` — `marked`-rendered `public/puzzle-info/puzzle-NN.md` inside design-system `Dialog`; reuses `src/web/puzzles/ChromaTreeExplorer.tsx` for puzzle-06 (FR-018)
- [ ] T049 [US1] Fill `src/web/screens/GrandCanvasScreen.tsx` (functional) — stats (puzzles solved / pets rescued / best streak), full pet roll, "Return to Studio", "Review & practise puzzles"; +200 applied once via domain (FR-020; redesign in US6)
- [ ] T050 [US1] Fill `src/web/screens/CollectionScreen.tsx` (functional) — simple pet grid from `usePets` (redesign in US5)
- [ ] T051 [US1] Implement `actions.autoSolveJourney()` — drives `submitPuzzle` with `getDemoSolution(puzzleId)` across the journey; dev/e2e only (FR-022)
- [ ] T052 [US1] Rewrite `src/web/main.tsx` — mount `<GameProvider><App/></GameProvider>` only; **remove `void import("./legacyGame")`** (the shell flip)
- [ ] T053 [US1] Reduce `index.html` to `<div id="root"></div>` + the module script; remove the static gameplay DOM skeleton, `#auto-solve`/`#reset` hooks and the `ctg:ready` event (FR-004)
- [ ] T054 [US1] Delete `src/web/legacyGame.ts` and `src/web/AppShell.tsx`; delete `src/web/legacy/infoModal.ts`, `src/web/legacy/learningFlow.ts`, `src/web/legacy/resultFeedback.ts` once T043/T046/T048 replace them (`src/web/legacy/artStationMiniGame.ts` stays until Phase 5); fix all imports (FR-002, FR-003)
- [ ] T055 [US1] Regression-check the CLI — `npm run play` (`help`/`status`/`list`), `npm run play solve puzzle-01`, `npm run play auto` behave exactly as before (shared domain untouched) (FR-023, SC-002)
